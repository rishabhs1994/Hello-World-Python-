...
try:
	url = address + '/users'
 	...
 	resp, content = h.request(url,'POST', body = data, headers = {"Content-" \
 		"Type": "application/json"})
	if resp['status'] != '201' and resp['status'] != '200':
 		raise Exception('Received status code %s' % resp['status'])
...
try:
	...
	if resp['status'] == '200':
		raise Exception("Security Bug: able to log in with wrong credentials")
except Exception as err:
	print "Test 2 FAILED"
	print err.args
	sys.exit()
...
try:
	...
	if resp['status'] != '200':
		raise Exception("Unable to view /api/resouce with valid credentials")
except Exception as err:
	print "Test 3 FAILED"
	print err.args
	sys.exit()
...
#TEST 4 TRY TO ACCESS '/home' WHICH IS RATE LIMITED TO SHOW EFFECT OF RATE 
#LIMITING 
try:
	h = Http()
	h.add_credentials("Rishabh", "Rishabh")
	...
	print "Test 4 FAILED: Logged in User can view /home without being Rate " \
	"Limited"
	sys.exit()

#TEST 5 TRY TO ACCESS '/api/users/1' WHICH IS NOT RATE LIMITED TO SHOW EFFECT 
#OF RATE LIMITING 
try:
	h = Http()
	h.add_credentials("Rishabh", "Rishabh")
	...
	print "Test 5 FAILED. User should have been able to see this page as ma" \
	"ny times as he want"
else:
